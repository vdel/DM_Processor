...
uint32_t read_op(list<string>  & slist) {
  ...
  // DONE check that the way is the allowed one
...
int main(int argc, char* argv[]){
  ...
  if (argc>1) {
    ...
    for(int i=1;i<argc;i++)
    {
      ...
      if(opt=="-v")
        verbose=1;
      if(string(opt, 0, 14)=="-base_address=") {
        opt.erase(0, 14);
        base_address=read_int(opt);
      }
      else if(opt=="-h")
	cout 
        << "Syntaxe: asm [options] source [target]\n" 
        << "The possible options are:\n"
        << "  -v:                  the program gives informations during compiling\n"
        << "  -base_address=<add>: set the base address at the address <add> (decimal or hexadecimal) (default: 0x5000)\n";
    }
  }
...
cout << base_address;
  ...
  // DONE  add the possibility to compile code to another address?
  ...
  for (uint32_t i=0; i<todo.size(); i++) {
	...
	exit(-1);  
      ...
      if(opcode==0x1e) { // jrf, delta should be positive
        //DM2 BUG
	if(/*delta<0 || delta>256*/delta<0 || delta>255) {
